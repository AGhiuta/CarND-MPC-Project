...
// The reference velocity is set to 60mph
double ref_v = 60;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N; t++) {
      fg[0] += coeff_penalize_cte * CppAD::pow(vars[cte_start + t], 2);
      fg[0] += coeff_penalize_epsi * CppAD::pow(vars[epsi_start + t], 2);
      fg[0] += coeff_penalize_v * CppAD::pow(vars[v_start + t] - ref_v, 2);
    }
    ...
    for (int t = 0; t < N - 2; t++) {
      fg[0] += coeff_deriv_delta * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += coeff_deriv_a * CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }
